...
execute as @s at @s run tellraw @a ["",{ "bold": false, "italic": false, "translate": "use.blade_of_truth.name", "fallback": "%1$s used \"Blade of Truth\".", "with": [ { "selector": "@s" }]}]
...
execute as @s[team=RED] at @s as @e[tag=BoT_TarBLU,distance=..25,sort=nearest,limit=1] at @s run particle dust_color_transition{from_color:[0.855,0.647,0.125],scale:1,to_color:[1.000,0.843,0.000]} ~ ~ ~ 5 3.5 5 1 1500 normal @a[distance=..30]
...
execute as @s[team=BLU] at @s as @e[tag=BoT_TarRED,distance=..25,sort=nearest,limit=1] at @s run particle dust_color_transition{from_color:[0.855,0.647,0.125],scale:1,to_color:[1.000,0.843,0.000]} ~ ~ ~ 5 3.5 5 1 1500 normal @a[distance=..30]
